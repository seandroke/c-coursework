...
void openFileWrite(char *fileName, char *toWrite){
	...
	struct infoSec temp = {""};
	fread(fileNameTemp, 1, 12, fp); 
	strcpy(temp.fileName, fileNameTemp);
	fread(extension, 1, 3, fp);
	strcpy(temp.extension, extension);
	fread(creationYear, 1, 4, fp);
	strcpy(temp.creationYear, creationYear);
	fread(creationMonth, 1, 2, fp);
	strcpy(temp.creationMonth, creationMonth);
	fread(creationDay, 1, 2, fp);
	strcpy(temp.creationDay, creationDay);
	fread(creationHour, 1, 2, fp);	
	strcpy(temp.creationHour, creationHour);
	fread(creationMinute, 1, 2, fp);
	strcpy(temp.creationMinute, creationMinute);
	fread(creationSecond, 1, 2, fp);
	strcpy(temp.creationSecond, creationSecond);
	strcpy(temp.modificationYear, getDateTime(1));
	strcpy(temp.modificationMonth, getDateTime(2));
	strcpy(temp.modificationDay, getDateTime(3));	
	strcpy(temp.modificationHour, getDateTime(4));
	strcpy(temp.modificationMinute, getDateTime(5));
	strcpy(temp.modificationSecond, getDateTime(6));
	fread(fileSize, 1, 6, fp);
	sscanf(fileSize, "%d", &fileSizeInt);
	totalFileSize = fileSizeInt + inputLength;
	sprintf(newFileSize, "%d", totalFileSize);
	strcpy(temp.fileSize, newFileSize);

	fseek(fp, (atol(infoSecRef) - 1) * 16, SEEK_SET);
	fwrite(&temp, sizeof(struct infoSec), 1, fp);
